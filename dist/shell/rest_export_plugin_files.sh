...
tmpzip="$(mktemp "${TMPDIR:-/tmp}/dbflux-rest-exppluginfiles.XXXXXX")"

headers=( "app_id:${DBFLOW_EXP_APP_ID}" "plugin_name:${DBFLOW_EXP_PLG_ID}" )
if [[ -n "${DBFLOW_EXP_FNAME}" ]]; then
  headers+=( "file_name:${DBFLOW_EXP_FNAME}" )
fi

rest_post_download "exppluginfiles" "${tmpzip}" "${headers[@]}"
if [[ $? -eq 0 ]]; then
  printf "${CLR_LBLUE}$(date '+%d.%m.%Y %H:%M:%S') >> Unzipping exported plugin files ... ${NC}\n"
